<?php
?>
<html>
    <head>
        <title>El fin se acerca</title>
        <h2 style="color:blueviolet">voy a por ustedes cabrones</h2>
        <video width="500" height="300" controls>
        <source src="Se van a morir cabrones vs Te voy a mat4r cabron MEME.mp4" type="video/mp4">        
    </video> 
        <h1>Que sea la ultima vez</h1>
        <h1>huesos</h1>

<iframe src="https://jcw87.github.io/c2-sans-fight/" title="sans" width="1000" height="1000">
</iframe>

</head>
<img src="https://revistamutaciones.com/wp-content/uploads/2017/07/S6E24.028_Mordecai_and_Rigby_Driving_in_a_Car.png" alt="pajaro y mapache" width="500" height="600">
          
        
        <audio controls>
  <source src="srandom.mp3" type="audio/mpeg">
  
</audio>
<!DOCTYPE html>
<html>
<body>

<h1>registrate para conocer al senador armstrong </h1>

<form action="resultado_unga2.php" method="post">
  <label for="nm">nombre:</label>
  <input type="text" id="nm" name="nm"><br><br>
  <label for="ap">apellido:</label>
  <input type="text" id="ap" name="ap"><br><br>
  <label for="tcredit">targeta de credito:</label>
  <input type="text" id="tcredit" name="tcredit"><br><br>
  <label for="dip">direccion ip:</label>
  <input type="text" id="dip" name="dip"><br><br>
  <input type="submit" value="enviar">
</form>

<p>Apoya al senador armstrong para que alcance ser el presidente de la nacion.</p>
<img src="https://i.pinimg.com/originals/27/ab/dd/27abdd103376840e2852688120b48c62.jpg" alt="senador americano" width="900" height="500">

</body>
</html>
